...
function unzip(source: string, target: string) {
  if (process.argv.some((arg) => arg.toLocaleLowerCase() === "--skipunzip"))
    return;
  console.log();
  ...
  return new Promise((resolve, reject) => {
    gunzipFile(source, target, () => {
      ...
      try {
        ...
        console.log(
          `${source} is ${(sourceStats.size / 1024 / 1024).toFixed(2)}MB`
        );
        console.log(
          `${target} is ${(targetStats.size / 1024 / 1024).toFixed(2)}MB`
        );
      } catch (err) {
